...
function isPrerelease(hostname: string | null): boolean {
    ...
    if (hostname === "docs.goauthentik.io") return true;
...
const VersionPickerLoader: React.FC<VersionPickerLoaderProps> = ({ pluginData }) => {
    ...
    const prereleaseOrigin = useMemo(() => {
        ...
        if (browser && (hostname === "localhost" || hostname === "127.0.0.1")) {
            return window.location.origin;
        }

        return "https://docs.goauthentik.io";
    }, [browser]);
    ...
    const hostname = useMemo(() => {
        ...
        // Navigating the page will de-sync the version picker.
...
export const VersionPicker: React.FC = () => {
    const pluginData = usePluginData("ak-releases-plugin") as AKReleasesPluginData;
